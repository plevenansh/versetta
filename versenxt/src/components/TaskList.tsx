...
export default function TaskList() {
  ...
  const handleCreateTask = async () => {
    if (!selectedTeamId || !newTask.projectId) return;
    try {
  ...
  <Select
    ...
    onValueChange={(value) => setNewTask({ ...newTask, subStageId: Number(value) })}
  >
    <SelectTrigger>
  ...
  >
    ...
    <SelectContent>
      {(() => {
        ...
        const mainStages = project.mainStages;
        const mainStage = mainStages.find(s => s.id === editingTask.mainStageId);
        if (!mainStage) return null;
        ...
        {editingTask.mainStageId && (
          ...
          >
            ...
            <SelectContent>
              {projects?.find(p => p.id === editingTask.projectId)?.mainStages
                .find(s => s.id === editingTask.mainStageId)?.subStages.map((subStage) => (
                  <SelectItem key={subStage.id} value={subStage.id.toString()}>
                    {subStage.name}
                  </SelectItem>
                ))}
