...
export default function VideoPage({ projectId }: { projectId: number }) {
  ...
  const { data: project, isLoading, error, refetch } = trpc.projects.getById.useQuery(projectId)
  ...
  useEffect(() => {
    if (project) {
      ...
      const completedSubStages = project.mainStages.reduce((acc, mainStage) => 
        acc + mainStage.subStages.filter(subStage => subStage.enabled).length, 0)
      const totalSubStages = project.mainStages.reduce((acc, mainStage) => 
